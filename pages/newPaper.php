...
<body class="flex flex-col h-screen justify-between overflow-hidden">
    ...
    <section class="text-gray-600 body-font font-mono relative overflow-auto">
        <div class="flex flex-col text-center w-full mb-2">
            <h2 class="text-xs text-green-500 tracking-widest font-semibold title-font">SUMMARY PAGE</h2>
            <h1 class="sm:text-2xl text-xl font-bold title-font text-gray-900"><?php echo $res->Title; ?></h1>
        </div>
        <form class="container px-5 py-2 mx-auto" method="post" action="../php-src/postPaper.php">
            <div class="w-full bg-white border-2 border-green-300 rounded-lg static sm:mr-10 p-2 flex flex-col justify-start relative">
                <div class="flex flex-wrap -m-2">
                    <div class="w-full">
                        <div class="relative">
                            <?php if (!(is_null($res->Title))) { ?>
                                <input type="hidden" id="pname" name="pname" class="w-full bg-white bg-opacity-50 rounded border border-gray-300 focus:border-green-500 focus:bg-white focus:ring-2 focus:ring-green-200 text-sm outline-none text-black py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" value="<?php echo $res->Title; ?>" readonly>
                            <?php } else { ?>
                                <input type="hidden" id="pname" name="pname" class="w-full bg-white bg-opacity-50 rounded border border-gray-300 focus:border-green-500 focus:bg-white focus:ring-2 focus:ring-green-200 text-sm outline-none text-black py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" required value="null">
                            <?php } ?>
                        </div>
                    </div>
                    <div class="lg:w-1/2 p-2 w-full">
                        <div class="relative">
                    ...
                    <div class="p-2 lg:w-1/4 w-full">
                        <div class="relative">
                            ...
                            <?php if (!(is_null($res->Publisher))) {
                                if (!(is_null($res->PublishedIn))) { ?>
                    ...
                    <div class="p-2 lg:w-1/2 w-full">
                        <div class="relative">
                            ...
                            <label for="upaper" class="leading-7 text-xs text-gray-600">(** Only Drive Link of paper)</label>
                    ...
                    <div class="p-2 lg:w-1/4 w-full">
                        <div class="relative">
                            ...
                            <?php if (!(is_null($res->PaperURL))) { ?>
                                <input type="hidden" id="purl" name="purl" value="<?php echo $res->PaperURL; ?>">
                                <a type="button" class="inline-flex text-black text-base bg-green-500 border-0 py-2 px-4 focus:outline-none hover:bg-green-700 rounded text-lg" name="purl" id="purl" href="<?php echo $res->PaperURL; ?>" target="_blank">
                                    Open Your Paper URL
                                    <i class="fa-solid fa-square-up-right text-xl p-1"></i>
                                </a>
                            <?php } else { ?>
                                <input type="hidden" id="purl" name="purl" value="<?php echo $res->PaperURL; ?>">
                    ...
                    <div class="p-2 w-full lg:w-1/2">
                        <label for="range" class="leading-7 text-sm text-gray-600">User Paper Rating</label>
                        <span id="rangeValue" name="rangeValue" class="font-bold text-sm">3</span>
            ...
            <div class="w-full bg-white border-0 border-green-200 rounded-md static sm:mr-10 flex flex-col my-1">
                <h2 class="text-gray-900 text-xl my-2 mx-auto font-semibold title-font">Your Summary</h2>
                <div class="p-2 w-full">
                    <input type="hidden" name="editor-content" id="editor-content">
                    <div id="editor" class="h-96 overflow-y-auto w-full bg-white rounded border border-gray-300 lg:h-52 h-40 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out"></div>

                    <div id="word-count" class="mt-1 text-right text-xs">0 words</div>
                    <div id="char-count" class="mt-1 text-right text-xs">0 characters</div>
                </div>
...
<script>
    ...
    quill.on('text-change', function () {
            ...
            // Add a warning class if word count exceeds 1500
            if (words > 1500) {
